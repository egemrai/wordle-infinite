...
function App() {
  ...
  function pushNewWord(data:word){
    if(words.includes(data.word)){
      ...
      (wordInput.current as HTMLInputElement).value = ("")
      setCurrentGuess('')
    }
    else{
  ...
  return (
    <>
      <div className="allPage" onClick={()=>wordInput.current?.focus()}>
        <div className="allWordsDiv ">
          <div className="wordDiv relative">
            ...
            <p id='errorMessage' className='errorMessage absolute'>türkçe öğren aptal köylü</p>
          </div>
